...
def findCellsWithSingleOption(sudoku:Sudoku) -> list[tuple[int,int]]:
    return [position for position,cell in sudoku.cells if cellHasOnePossibleNumber(cell)]
...
def findCellsWithUniqueNumberInRows(sudoku:Sudoku) -> list[tuple[int,int]]:
    return [position for y in range(1, 10) for position in findCellsWithUniqueNumberInSingleRow(sudoku.getrow(y))]
...
def findCellsWithUniqueNumberInColumns(sudoku:Sudoku) -> list[tuple[int,int]]:
    return [position for x in range(1, 10) for position in findCellsWithUniqueNumberInSingleColumn(sudoku.getcolumn(x))]
...
def findIndexesForUniqueNumbersInLine(line:Line) -> list[int]:
    ...
    return [index for l in numbers if len(numbers[l]) == 1 for index in numbers[l]]

def findCellsWithUniqueNumberInBlocks(sudoku:Sudoku) -> list[tuple[int,int]]:
    return [position for (x,y) in itertools.product(range(1, 4), range(1, 4)) for position in findCellsWithUniqueNumberInSingleBlock(sudoku.getblock(x, y))]

def findCellsWithUniqueNumberInSingleBlock(block:Block) -> list[tuple[int,int]]:
    numbers = {key:list() for key in range(1, 10)}
    for (x, y, cell) in [(x, y, block.get(x, y)) for (x,y) in itertools.product(range(1, 4), range(1, 4))]:
        if isinstance(cell, EmptyCell):
            for possibleNumber in cell.possibleNumbers:
                numbers[possibleNumber].append((x, y))
    return [block.localToGlobal(x, y) for l in numbers if len(numbers[l]) == 1 for x,y in numbers[l]]